...
// Add to Cart
  if (e.target.classList.contains("add-btn")) {
    ...
    // Persist only if authenticated; otherwise prompt user later at checkout
    try {
      const token = localStorage.getItem('authToken');
      if (token && window.Auth) {
        Auth.fetchWithAuth('/api/order', {
          method: 'POST',
...
async function checkout() {
  ...
  const proceed = async () => {
    try {
      ...
      const resp = await Auth.fetchWithAuth('/api/order', {
        method: 'POST',
        body: JSON.stringify({ items })
      });
      const data = await resp.json();
      if (!resp.ok || !data.success) throw new Error(data.message || 'Order failed');
